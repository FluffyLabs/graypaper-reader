...
import {INoteV2, exportNotesAsJson, importNotesFromJson} from './notesImportExport'
...
test('should import v2 notes', () => {
  const notesV2: INoteV2[] = [{"content":"Here it seems that even if the value is smaller than `b_z` we still fail if the memory there is not writable!","date":1729929377597,"author":"","pageNumber":46,"selectionStart":{"pageNumber":46,"index":569},"selectionEnd":{"pageNumber":46,"index":578},"selectionString":"if v ≠ ∅ ∧ Zbo⋅⋅⋅+bz ⊂ V∗\nµ","version":"911af30f94dad0c7784d60d568d9c032517821ee"}
  ];
  ...
  expect(importNotesFromJson(notesStr, 'must-have-label')).toStrictEqual({
    ...
    notes: [{
      noteVersion: 3,
      content: "Here it seems that even if the value is smaller than `b_z` we still fail if the memory there is not writable!",
      date: 1729929377597,
      author: "",
